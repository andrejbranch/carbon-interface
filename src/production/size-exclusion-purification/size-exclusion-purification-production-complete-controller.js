angular.module('production.sizeExclusionPurification.sizeExclusionPurificationProductionCompleteCtrl', [])

    .controller('sizeExclusionPurificationProductionCompleteCtrl', ['$scope', 'purificationRequest', '$cbResource', 'sampleGridFactory', 'API', 'purificationSampleType', '$localStorage', 'sampleImportManager', 'StepsService',

        function ($scope, purificationRequest, $cbResource, sampleGridFactory, API, purificationSampleType, $localStorage, sampleImportManager, StepsService) {

            var fileInput;

            $scope.sizeExclusionPurificationRequest = purificationRequest;
            $scope.sampleType = purificationSampleType;
            $scope.importForm = {};
            ...
            $scope.selectTotalSamples = function (totalSamples) {
                ...
                StepsService.steps('purificationProductionComplete').next();
            }
            ...
            $scope.download = function () {
                ...
                $cbResource.get('/production/purification-request/' + $scope.sizeExclusionPurificationRequest.id + '/download-output-template/' + $scope.totalSamples).then(function (response) {
                    ...
                    var filename = 'Purification Request ' + $scope.sizeExclusionPurificationRequest.id + ' Template.csv';
            ...
            $scope.handleUpload = function () {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (xhr.readyState == 4 && xhr.status == 200 && xhr.responseText) {
                        ...
                        StepsService.steps('purificationProductionComplete').goTo(2);
            ...
            $scope.import = function () {
                ...
                angular.forEach($scope.importGrid.data, function (sample) {
                    ...
                    sample.lot = $scope.sizeExclusionPurificationRequest.alias;
                ...
                $cbResource.create('/storage/sample-import/save', bulkSamples).then(function (response) {
                    ...
                    $scope.isUploading = false;
                    StepsService.steps('purificationProductionComplete').goTo(4);

                    $scope.sampleIds = response.data;

                    $cbResource.create('/production/purification-request/' + $scope.sizeExclusionPurificationRequest.id + '/complete', $scope.sampleIds).then(function (response) {
