angular.module('production.secMals.secMalsProductionCompleteCtrl', [])

    .controller('secMalsProductionCompleteCtrl', ['$scope', 'secMalsRequest', '$cbResource', 'sampleGridFactory', 'API', 'secMalsSampleType', '$localStorage', 'sampleImportManager', 'StepsService',

        function ($scope, secMalsRequest, $cbResource, sampleGridFactory, API, secMalsSampleType, $localStorage, sampleImportManager, StepsService) {

            var fileInput;

            $scope.secMalsRequest = secMalsRequest;
            $scope.sampleType = secMalsSampleType;
            $scope.importForm = {};
            ...
            $scope.selectTotalSamples = function (totalSamples) {
                ...
                StepsService.steps('secMalsProductionComplete').next();
            }
            ...
            $scope.download = function () {
                ...
                $cbResource.get('/production/secMals-request/' + $scope.secMalsRequest.id + '/download-output-template/' + $scope.totalSamples).then(function (response) {
                    ...
                    var filename = 'Gel Request ' + $scope.secMalsRequest.id + ' Template.csv';
            ...
            $scope.handleUpload = function () {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (xhr.readyState == 4 && xhr.status == 200 && xhr.responseText) {
                        ...
                        StepsService.steps('secMalsProductionComplete').goTo(2);
            ...
            $scope.import = function () {
                ...
                angular.forEach($scope.importGrid.data, function (sample) {
                    ...
                    sample.lot = $scope.secMalsRequest.alias;
                ...
                $cbResource.create('/storage/sample-import/save', bulkSamples).then(function (response) {
                    ...
                    $scope.isUploading = false;
                    StepsService.steps('secMalsProductionComplete').goTo(4);

                    $scope.sampleIds = response.data;

                    $cbResource.create('/production/secMals-request/' + $scope.secMalsRequest.id + '/complete', $scope.sampleIds).then(function (response) {
