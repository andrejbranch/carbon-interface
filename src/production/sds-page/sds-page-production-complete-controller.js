angular.module('production.sdsPage.sdsPageProductionCompleteCtrl', [])

    .controller('sdsPageProductionCompleteCtrl', ['$scope', 'sdsPageRequest', '$cbResource', 'sampleGridFactory', 'API', 'sdsPageSampleType', '$localStorage', 'sampleImportManager', 'StepsService',

        function ($scope, sdsPageRequest, $cbResource, sampleGridFactory, API, sdsPageSampleType, $localStorage, sampleImportManager, StepsService) {

            var fileInput;

            $scope.sdsPageRequest = sdsPageRequest;
            $scope.sampleType = sdsPageSampleType;
            $scope.importForm = {};
            ...
            $scope.selectTotalSamples = function (totalSamples) {
                ...
                StepsService.steps('sdsPageProductionComplete').next();
            }
            ...
            $scope.download = function () {
                ...
                $cbResource.get('/production/sdsPage-request/' + $scope.sdsPageRequest.id + '/download-output-template/' + $scope.totalSamples).then(function (response) {
                    ...
                    var filename = 'Gel Request ' + $scope.sdsPageRequest.id + ' Template.csv';
            ...
            $scope.handleUpload = function () {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (xhr.readyState == 4 && xhr.status == 200 && xhr.responseText) {
                        ...
                        StepsService.steps('sdsPageProductionComplete').goTo(2);
            ...
            $scope.import = function () {
                ...
                angular.forEach($scope.importGrid.data, function (sample) {
                    ...
                    sample.lot = $scope.sdsPageRequest.alias;
                ...
                $cbResource.create('/storage/sample-import/save', bulkSamples).then(function (response) {
                    ...
                    $scope.isUploading = false;
                    StepsService.steps('sdsPageProductionComplete').goTo(4);

                    $scope.sampleIds = response.data;

                    $cbResource.create('/production/sdsPage-request/' + $scope.sdsPageRequest.id + '/complete', $scope.sampleIds).then(function (response) {
