angular.module('production.nativeGel.nativeGelProductionCompleteCtrl', [])

    .controller('nativeGelProductionCompleteCtrl', ['$scope', 'nativeGelRequest', '$cbResource', 'sampleGridFactory', 'API', 'nativeGelSampleType', '$localStorage', 'sampleImportManager', 'StepsService',

        function ($scope, nativeGelRequest, $cbResource, sampleGridFactory, API, nativeGelSampleType, $localStorage, sampleImportManager, StepsService) {

            var fileInput;

            $scope.nativeGelRequest = nativeGelRequest;
            $scope.sampleType = nativeGelSampleType;
            $scope.importForm = {};
            ...
            $scope.selectTotalSamples = function (totalSamples) {
                ...
                StepsService.steps('nativeGelProductionComplete').next();
            }
            ...
            $scope.download = function () {
                ...
                $cbResource.get('/production/nativeGel-request/' + $scope.nativeGelRequest.id + '/download-output-template/' + $scope.totalSamples).then(function (response) {
                    ...
                    var filename = 'Gel Request ' + $scope.nativeGelRequest.id + ' Template.csv';
            ...
            $scope.handleUpload = function () {
                ...
                xhr.onreadystatechange = function () {
                    ...
                    if (xhr.readyState == 4 && xhr.status == 200 && xhr.responseText) {
                        ...
                        StepsService.steps('nativeGelProductionComplete').goTo(2);
            ...
            $scope.import = function () {
                ...
                angular.forEach($scope.importGrid.data, function (sample) {
                    ...
                    sample.lot = $scope.nativeGelRequest.alias;
                ...
                $cbResource.create('/storage/sample-import/save', bulkSamples).then(function (response) {
                    ...
                    $scope.isUploading = false;
                    StepsService.steps('nativeGelProductionComplete').goTo(4);

                    $scope.sampleIds = response.data;

                    $cbResource.create('/production/nativeGel-request/' + $scope.nativeGelRequest.id + '/complete', $scope.sampleIds).then(function (response) {
